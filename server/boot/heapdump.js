...
module.exports = function HeapDumpFn(app, cb) {
  if (process.env.HEAPDUMP_INTERVAL && ms) {
    var heapdumpGap = ms(process.env.HEAPDUMP_INTERVAL);
    if (heapdumpGap) {
      app.once('started', function appStartedEventCb(appinstance) {
        ...
        setInterval(writeHeapdump, heapdumpGap);
      ...
      log.error(log.defaultContext(), 'HEAPDUMP_GAP env variable is not in proper format. Check @ms node_module for supported format.');
    }
    return cb();
  }
  cb();
