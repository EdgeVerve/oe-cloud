...
describe(chalk.blue('Decision table belongsTo relation check'), function () {
        ...
        models.ModelDefinition.events.once('model-' + childModelName + '-available', function () {
            models.DecisionTable.create({
                ...
                models.ModelRule.create(modelRuleData, bootstrap.defaultContext, function(err, result){
                    ...
                    expect(result.modelName).to.be.equal(childModelName);
                    done();
                });
            });
        });
    ...
    it('Successfully creates childModel data as belongsTo validation passes', function (done) {
        ...
        models[parentModelName].create(parentPayload, bootstrap.defaultContext, function(err, parentInst){
            if(err){
                return done(err);
            }
            models[childModelName].create(childPayload, bootstrap.defaultContext, function(err, childInst){
                if(err){
                    return done(err);
                }
