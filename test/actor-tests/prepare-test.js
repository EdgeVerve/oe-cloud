...
fs.writeFile("common/models/framework/inventory.js", inventoryJs, function(err) {
    if(err) {
        return process.exit(-1);
    }
});
fs.writeFile("common/models/framework/inventory.json", inventoryJson, function(err) {
    if(err) {
        return process.exit(-1);
    }
});
fs.writeFile("common/models/framework/inventoryTransaction.js", inventoryTransactionJs, function(err) {
    if(err) {
        return process.exit(-1);
    }
});
fs.writeFile("common/models/framework/inventoryTransaction.json", inventoryTransactionJson, function(err) {
    if(err) {
        return process.exit(-1);
    }
}); 
