...
describe(chalk.blue('Embed many test'), function () {
  ...
  before('create models  with embed many relations', function (done) {
    ...
    models.ModelDefinition.create([model2, model3], bootstrap.defaultContext, function (err, res) {
      if (err) {
        ...
        accountModel = loopback.getModel('Account', bootstrap.defaultContext);
        customerModel = loopback.getModel('CustomerEmbed', bootstrap.defaultContext);
        evfUserModel = loopback.getModel('EVFUser', bootstrap.defaultContext);
        done();
      }
    });
  });
  ...
  after('clean up-', function (done) {
    customerModel.destroyAll({}, bootstrap.defaultContext, function (err, res) {
      console.log(err, '\n', res);
  ...
  it('should create a evfUser with account details -- programmatically', function (done) {
    ...
    customerModel.create(data, bootstrap.defaultContext, function (err, res) {
      if (err) {
        ...
        customerModel.findById(res.id, bootstrap.defaultContext, function (err, res) {
          if (err) {
            done(err);
          } else {
            expect(res).not.to.be.null;
