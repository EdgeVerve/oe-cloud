...
describe(chalk.blue('Aggregation Functions with group filter test'), function () {
  ...
  before('Create Test model and upload test data', function (done) {
    models.ModelDefinition.create(marksListDef, bootstrap.defaultContext, function (err, res) {
      if (err) {
        ...
        marksList = loopback.getModel('StudentsMarksList', bootstrap.defaultContext);
  ...
  describe(chalk.green('Aggregation Functions having filter Test --REST'), function () {
  ...
  describe(chalk.green('Aggregation Functions having filter Test --Programatic'), function () {
