...
var log = require('evf-logger')('BaseActorEntity');
...
module.exports = function (BaseActorEntity) {
  ...
  BaseActorEntity.prototype.createMessage = function (activity, journalEntityType, journalEntityVersion, journalEntityTime, journalEntityId) {
    var message = {};
    ...
    message.jounalId = journalEntityId;
  ...
  BaseActorEntity.prototype.reserveAmount = function (context) { // should be async
    ...
    var journalEntityId = journalEntity._id;
    var journalEntityTime = journalEntity._createdOn || journalEntity._modifiedOn;

    var message = this.createMessage(context.activity, journalEntityType, journalEntityVersion, journalEntityTime, journalEntityId);
  ...
  BaseActorEntity.prototype.processMessagesBackground = function (envelope, options, actorCb) {
    ...
    stateModel.findById(this.stateId, options, function (err, stateObj) {
      ...
      async.eachSeries(messages, function (message, cb) {
        ...
        self.constructor.instanceLocker().acquire(self, options, self._version, function (releaseLockCb) {
          stateObj.updateAttributes(stateObj.__data, options, function (error, info) {
            if (error) {
  ...
  BaseActorEntity.prototype.processMessage = function (envelope, message, state, options, cb) {
    ...
    async.eachSeries(loopbackModelsCollection, function (model, asyncCB) {
      model.findById(message.jounalId, options, function (err, result) {
        if (err) {
          return asyncCB(err);
        } else if (!result) {
          return asyncCB();
    ...
    stateModel.findById(this.stateId, options, function (err, state) {
      ...
      async.each(loopbackModelsCollection, function (model, asyncCb) {
        model.find(query, options, function (err, returnedInstances) {
          ...
          envelope.seqNum = state.seqNum;
          state.updateAttributes(state.__data, options, function (error, info) {
            if (error) {
              log.error(options, 'error while persisting actor ', error);
  ...
  BaseActorEntity.prototype.drainMailBox = function (activity, options, cb) {
    if (options.lockMode === DB_LOCK_MODE) {
      ...
      if (envelope === null || envelope === undefined) {
        ...
        actorPool.getOrCreateInstance(context, options, cb);
      } else {
        processEnvelope();
      }
    } else {
  ...
  var updateStateId = function (ctx, instance, value, cb) {
    var instanceModel = loopback.findModel(instance._type);
    instanceModel.findById(instance.id, ctx.options, function (err, inst) {
      if (err) {
        cb(err);
      } else {
        inst.stateId = value;
        inst.updateAttribute('stateId', value, ctx.options, function (err, res) {
          if (err) {
