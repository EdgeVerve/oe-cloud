...
var applyPersonalizationRule = function applyPersonalizationRuleFn(ctx, p13nRule, callback) {
  ...
  log.debug(ctx.req.callContext, 'applying Personalizing ctx with function - ', JSON.stringify(p13nRule));
...
function fieldReplacementFn(ctx, replacements) {
  ...
  } else if (ctx.req.body !== undefined && !_.isEmpty(ctx.req.body)) {
    input = ctx.req.body;
...
function addPostProcessingFunction(ctx, func, instruction, fn) {
  ...
  callContext.postProcessingFns[callContext.modelName] = callContext.postProcessingFns[ctx.Model.modelName] || [];

  if (func === 'fieldReplace') {
    callContext.postProcessingFns[callContext.modelName].push(new ProcessingFunction(instruction, fn));
  } else {
    callContext.postProcessingFns[callContext.modelName].unshift(new ProcessingFunction(instruction, fn));
  }
