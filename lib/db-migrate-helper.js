...
function appVerGreaterThanDBVer(appVer, dbVer) {
  ...
  appVerGTDBVer = Object.keys(appVer).some((key) => {
    ...
    if (typeof dbVer[key] === 'undefined') {
      return true;
    } else if (compareVersions(appVerVal, dbVerVal) === 1) {
      return true;
    }
  });
...
// Compares two semver versions and returns 1 if gt, 0 if eq, -1 if lt
function compareVersions(ver1, ver2) {
  var hyphenPres1 = ver1.indexOf('-') > -1;
  var hyphenPres2 = ver2.indexOf('-') > -1;
  var ver1Arr = buildSemverArray(hyphenPres1, ver1);
  var ver2Arr = buildSemverArray(hyphenPres2, ver2);
  ver1Arr = convertToNum(ver1Arr);
  ver2Arr = convertToNum(ver2Arr);
  var len = Math.max(ver1Arr.length, ver2Arr.length);
  for (var i = 0; i < len; i++) {
    if (typeof ver1Arr[i] === 'undefined') {
      return -1;
    } else if (typeof ver2Arr[i] === 'undefined') {
      return 1;
    }
...
// Builds semver array from the given semver value
function buildSemverArray(flag, version) {
  var result = [];
  if (flag) {
