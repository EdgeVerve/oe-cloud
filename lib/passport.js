...
module.exports.configurePassport = function configurePassport(app, passportConfigurator, providerConfigParameter) {
  ...
  userIdentity.observe('after save', function(ctx, next) {
    if (ctx.instance.authScheme === 'ldap') {
      ...
      function baseRoleMappingCb(err, roleMappings) {
        if (err) {
          console.error(err);
          return next(err);
        }
      ...
      function ldapRoleMappingCb(err, ldapRoleMappings) {
        if (err) {
          console.error(err);
          return next(err);
        }
        console.log('found ldapRoleMappings:', ldapRoleMappings);
        roleMappings = createRoleMappings(ldapRoleMappings, principalId);
      ...
      function createRoleMappings(ldapRoleMappings, principalId) {
