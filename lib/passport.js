...
module.exports.configurePassport = function configurePassport(app, passportConfigurator, providerConfigParameter) {
  ...
  // Providers.json will now be looked only from app if app is using ev foundation, if ev foundation is directly run as app, then it will be picked up from framework (so app will not fallback on evfoundation to avoid by chance picking unintentional configuration)
  var config = {};
