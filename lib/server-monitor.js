var EventEmitter = require('events');
var emitter = new EventEmitter();
module.exports.eventEmitter = emitter;
var logger = require('oe-logger');
var log = logger('server-monitor');

var time1; var time2; var currentLag;
var load;

const MEASUR_SPACE    = 5;
const MEASUR_INTERVAL = 10;
const EVENT_INTERVAL  = 5 * 1000;


class Average {
  constructor() {
    this.maxSize = 1000;
    this.queue = new Array(this.maxSize);
    this.size = 0;
    this.sum = 0;
    this.h = 0;
    this.t = 0;
  }

  update(val) {
    if (this.size === this.maxSize) {
      this.sum -= this.queue[this.t];
      this.t = this.t === this.maxSize - 1 ? 0 : this.t + 1;
    } else {
      this.size++;
    }
    this.queue[this.h] = val;
    this.h = this.h === this.maxSize - 1 ? 0 : this.h + 1;
    this.sum += val;
  }

  getAvg() {
    return this.sum / this.size;
  }
}

var average = new Average();


module.exports.init = () => {
  console.log('At init');
  setInterval(getEventLoopLegacy, MEASUR_INTERVAL);
  setInterval(setServerLoad, EVENT_INTERVAL);
};

var getEventLoopLegacy = () => {
  time1 = new Date().getTime();
  setTimeout(() => {
    time2 = new Date().getTime();
    currentLag = time2 - time1 - MEASUR_SPACE;
    average.update(currentLag);
    // console.log('lag,' + currentLag + ',avg,' + average.getAvg() + ',load,' + load);
  }, MEASUR_SPACE);
};

var setServerLoad = () => {
  var avg = average.getAvg();

  switch (true) {
    case (avg < 4):
      reportLoad('LOW');
      break;
    case (avg >= 4 && avg <= 6):
      reportLoad('MEDIUM');
      break;
    case (avg > 6):
      reportLoad('HIGH');
      break;
    default:
      break;
  }

  // console.log('lag,' + average.queue[average.h - 1] + ',avg,' + avg + ',load,' + load);
};

var reportLoad = (currentLoad) => {
  if (currentLoad !== load) {
    var msg = 'SERVER_LOAD_' + currentLoad;
    // console.log(msg);
    log.info(log.defaultContext, msg);
    emitter.emit(msg);
  }
  load = currentLoad;
};
