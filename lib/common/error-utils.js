 ...
 *
 * @function ErrorUtils
 * @param {string} code - function to be called once validation is over
...
function getValidationError(code, obj, cb) {
  ...
  Model.findOne(filter, obj.options, function errorUtilsModelFindCb(err, result) {
    if (err) {
      log.error(obj.options, 'Error in getting error model ', filter);
    }
    var errorObj = {};
    var moreInfoUrl = {};
...
function attachValidationError(instance, errArr) {
  ...
  errArr.forEach(function modelValidationsGetErrorForEachFn(err) {
    ...
    errObj.path = err.path ? err.path + '->' + err.fieldName : err.path;
        /*			if (err.index) {
