  ...
  if (ctxStr !== createDefaultContextString(autoscopeFields)) {
    if (jsonifyModel.relations) {
      Object.keys(jsonifyModel.relations).forEach(function (r) {
        ...
        if (!app.models[jsonifyModel.relations[r].clientModel] && createContextString(autoscopeFields, actualContext.ctx) !== createDefaultContextString(autoscopeFields)) { jsonifyModel.relations[r].model = jsonifyModel.relations[r].model + '-' + createContextString(autoscopeFields, actualContext.ctx); }
      });
    }
    let typeList = ['string', 'number', 'date', 'boolean', 'object', 'email', 'timestamp'];
