...
module.exports.checkModelWithPlural = function utilCheckModelWithPlural(server, plural) {
  ...
  for (var i = 0; i < models.length; ++i) {
    if (models[i].clientPlural) {
      if (models[i].clientPlural === plural || models[i].pluralModelName === plural) {
        return models[i].clientModelName;
      }
    } else if (models[i].pluralModelName === plural) {
...
module.exports.createModel = function utilCreateModel(app, modeldefinition, options, cb) {
  ...
  jsonifyModel.clientModelName = jsonifyModel.name;
  ...
  if (ctxStr === createDefaultContextString(autoscopeFields)) {
    app.personalizedModels[jsonifyModel.name][ctxStr] = {
      'modelId': jsonifyModel.name, 'context': ctxStr
    };
  } else {
    app.personalizedModels[jsonifyModel.name][ctxStr] = {
      'modelId': jsonifyModel.modelId, 'context': ctxStr
    };
  ...
  if (ctxStr !== createDefaultContextString(autoscopeFields)) {
    if (jsonifyModel.relations) {
      Object.keys(jsonifyModel.relations).forEach(function (r) {
        ...
        if (!app.models[jsonifyModel.relations[r].clientModel])
    ...
    if (jsonifyModel.properties) {
      Object.keys(jsonifyModel.properties).forEach(function (p) {
        if (jsonifyModel.properties[p].type === 'string' || jsonifyModel.properties[p].type === 'number'
          || jsonifyModel.properties[p].type === 'boolean' || jsonifyModel.properties[p].type === 'date' || jsonifyModel.properties[p].type === 'object') {
        }
        else if (typeof (jsonifyModel.properties[p].type) === 'string') {
        ...
        else if (Array.isArray(jsonifyModel.properties[p].type)) {
          var embeddedType = jsonifyModel.properties['addresses'].type[0];
          if (embeddedType === 'string' || embeddedType === 'number'
            || embeddedType === 'boolean' || embeddedType === 'date' || embeddedType === 'object') {
          }
          else {
            if (!app.models[embeddedType])
              jsonifyModel.properties[p].type[0] = jsonifyModel.properties['addresses'].type[0] + '-' + createContextString(autoscopeFields, actualContext.ctx);
          }
        }
      });
    }
  ...
  if (mds) {
    ...
    // TODO should do this inside connector
    mds.autoupdate(model.modelName, function (err, result) {
      if (err) {
...
var createCollectionWithContext = (autoscope = [], context) => {
  var ctxStr = [];
  autoscope.forEach(function (item, index) {
    var ctxVal = context[item];
    if (!ctxVal) {
...
module.exports.createCollectionWithContext = createCollectionWithContext;
