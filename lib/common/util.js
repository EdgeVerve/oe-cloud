...
const isBaseChainPersistedModel = (model, options) => {
  if (model && model.base && model.base.modelName && model.base.modelName !== 'Model') {
    // Checking the Model is 'PersistedModel' or not.
    if (model.base.modelName === 'PersistedModel') {
      return true;
    }
    // Get the new Model from the base.
    let newModel = loopback.findModel(model.base.modelName, options);
