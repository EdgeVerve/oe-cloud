    ...
    if (err) {
      ...
      switch (relation.type) {
        case 'belongsTo':
          ...
          Model.findOne(filter, obj.options, function validateBelongsToFindCb(error, result) {
            if (contextModified) {
              obj.options.ctx = orgctx;
            }
...
function validateRefCode(obj, cb) {
  ...
  if (blank(obj.data[obj.name])) {
    ...
    refCodeModel = loopback.findModel(obj.value, obj.options);
    if (refCodeModel) {
      refCodeModel.findOne({
        'fields': { 'code': true },
        'where': { 'code': obj.data[obj.name] }
  ...
  } else if (Array.isArray(obj.data[obj.name])) {
    refCodeModel = loopback.findModel(obj.value, obj.options);
    if (refCodeModel) {
      refCodeModel.find({
        'fields': { 'code': true },
        'where': { 'code': { 'inq': obj.data[obj.name] } }
      }, obj.options, function validateRefCodeCb(err, results) {
        var notThere = _.difference(obj.data[obj.name], results.map(refcode => refcode.code));
        if (results.length > 0 && notThere.length < 1) {
          log.debug(obj.options, 'validateRefCode rule passed for ', obj.inst.constructor.modelName);
          cb(null, null);
        } else {
          log.warn(obj.options, 'validateRefCode rule violated');
          obj.inst[obj.name] = notThere;
          getValidationError(code, obj, function validateRefCodeGetErrCb(error) {
            log.debug(obj.options, 'validateRefCode error :', error);
            err = error;
            cb(null, err);
