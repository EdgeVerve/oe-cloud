...
var useBroadcaster = false;

var orchestrator = process.env.ORCHESTRATOR;
if (orchestrator || process.env.USE_BROADCASTER) {
  useBroadcaster = true;
}

if (useBroadcaster) {
  log.info(log.defaultContext(), 'Using broadcaster ');
}
...
function init() {
  ...
  ws.on('open', function open() {
    log.info(log.defaultContext(), 'connection opened to broadcaster');
    registerNode();
    setPingTimer();
  });
    ...
    for (var key in interfaces) {
      if (interfaces.hasOwnProperty(key)) {
        ...
        for (var i = 0; i < networkInterface.length; i++) {
          ...
          if (networkAddress.family === 'IPv4' && networkAddress.netmask === '255.255.255.0' && matchIP.exec(ip) ) {
            return cb(null, ip);
          }
        }
      }
