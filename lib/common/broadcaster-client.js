...
function getIP(cb) {
  ...
  dns.lookup(routerHost, {}, function (err, routerIP, family) {
    if (family === 6) {
      log.error('IPV6 not supported as address for router');
      var error = new Error('router service discovery does not support IPV6 yet');
      return cb(error);
    }
    var ipArray = routerIP.split('.');
    ipArray.splice(ipArray.length-1,1,'[0-9]{1,3}');
    var matchIP = new RegExp(ipArray.join('\\.'));
    console.log('regex: ', matchIP)
    var interfaces = os.networkInterfaces();
    for (var key in interfaces) {
      var interface = interfaces[key];
      var ipv4 = interface[1].address;
        console.log(ipv4);
      if (matchIP.exec(ipv4)) {
        return cb(null, ipv4);
      }
    }
    log.error('Router network interface not found');
    var error = new Error('Router network interface not found');
    cb(error);
  });
}
