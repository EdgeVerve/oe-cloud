...
//        serverDir = './server';
//        if (appitem.serverDir) {serverDir = appitem.serverDir;}
...
function mergeCustomModels(allModels, modelPathMap, options) {
  var customModels = Object.keys(allModels).filter(function (k) {
    return allModels[k].customModel;
  });

  for (var i = 0; i < customModels.length; ++i) {
    let mergedModel = {};
    let name = customModels[i];
    let modelPaths = modelPathMap[name];
    let save = {};
    for (var j = 0; j < modelPaths.length; ++j) {
      var s = modelPaths[j];
      ...
      if (fs.existsSync(p + '.json')) {
        log.debug(log.defaultContext, 'Merging ' + name, p + '.json');
        var model = require(p + '.json');
        save[name + '_' + j.toString()] = model;
        _.merge(mergedModel, model);
      }
    }
    for (var k in save) {
      if (save[k]) {_.merge(save[k], mergedModel);}
    }
  }
}
...
var loadAppList = function loadAppList(applist, dir, bootoptions) {
  ...
  mergeCustomModels(allModels, modelPathMap, options);
