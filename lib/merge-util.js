...
var addTriggersToDatasources = function addTriggersToDatasourcesFn(mergedDataSources) {
  var triggers = require('../resources/triggers.json');
  for (var t in triggers) {
    if (triggers.hasOwnProperty(t)) {
      var datasourceConfig = mergedDataSources[triggers[t].datasourceName];
      if (typeof datasourceConfig !== 'undefined') {
        datasourceConfig.triggers = datasourceConfig.triggers ? datasourceConfig.triggers : {};
        var config = {};
        config[t] = {};
        config[t] = triggers[t].triggerStr;
        mergeDataSourcesObjects(datasourceConfig.triggers, config);
      }
    }
  }
};
...
module.exports = {
  ...
  addTriggersToDatasources: addTriggersToDatasources
};
