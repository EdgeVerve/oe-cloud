...
var log = require('evf-logger')('eventHistoryManager');
...
function handleEventHistory(eventHistory) {
  ...
  if (eventHistory.retryTimes === MAX_RETRY) { // TODO save to another collection for visibility
  ...
  checkModelInstance(model, version, function (error, vaild) {
    if (!vaild) {
...
function updateEventHistory(modelName, version, observerId) {
  ...
  var eventHistory = _.find(eventHistoryArray, function (event) {return event.modelName === modelName && event.version === version;});
    ...
    eventHistoryModel.find({where: {hostName: os.hostname()}}, ignoreScopeOptions, function (err, instances) {
      if (err) {
        ...
        eventHistoryModel.findOne({}, {fetchAllScopes: true, ctx: {tenantId: 'default'}}, function (err, instance) {
          if (err) { // TO DO - error handling here
            return cb();
          } else if (instance) {
            fetchModels(appInstance, instance.timestamp, function (error) {
              eventReplayJobInterval = setInterval(eventReplayJob, REPLAY_JOB_INTERVAL);
              persistToDBInterval = setInterval(persistToDB, PRESIST_TO_DB_INTERVAL);
              firstInit = false;
              cb();
      ...
      Model.find(query, {fetchAllScopes: true, ctx: {tenantId: 'default'}}, function (err, results) {
        ...
        if (results) {
          results.forEach(function (rec) {
            ...
            createEventHistory(rec._type, rec._version, ctx);
          }, this);
          callback(err);
        }
      });
