...
myEE.on('event reliability recovery', function (recoveryHostNameArr) {
  console.log('--------- Host: ', process.env.HOSTNAME, ' recieved hosts: ', recoveryHostNameArr, ' for recovery ---------');
  async.each(recoveryHostNameArr, function (hostname, asyncCb) {
    recovery(hostname, asyncCb);
  }, function (err) {
    if (err) {
      console.log('--------- error occured during recovery ---------');
      log.debug(log.defaultContext(), 'error while recovery of dead nodes', err);
      // TO DO - error handling here
    } else {
      console.log('--------- Recovery for hosts: ', recoveryHostNameArr, ' finished ---------');
      log.debug(log.defaultContext(), 'finished recovery for all dead nodes');
      // TODO: publish hostname recovered successfully
    }
  });
});

function recovery(hostname, cb) {
  console.log('--------- Starting recovery for host: ', hostname, ' ---------');
  var eventHistoryModel = getEventHistoryModel();
...
function fetchLimit(Model, query, property, cb) {
  Model.find(query, { fetchAllScopes: true, ctx: { tenantId: 'default' } }, function (err, results) {
    ...
    if (results) {
      ...
      if (results.length !== 0) {
        console.log('--------- For model: ', Model.modelName, ' retrieved: ', results.length, ' results ---------');
        var maxTime = results[results.length - 1][property];
        console.log('--------- For model: ', Model.modelName, ' new maxTime for property: ', property, ' is: ', maxTime);
      }
...
function fetchModels(hostNameToRecover, timeToRecoverPerModelMap, cb) {
  console.log('--------- Fetching instances for host: ', hostNameToRecover, 'with times: ', timeToRecoverPerModelMap, ' ---------');
  ...
  async.each(Object.keys(app.models), function (model, callback) {
    if (app.models[model].definition.settings.mixins &&
      ...
      if (typeof timeToRecoverOld !== 'undefined') {
        ...
        console.log('--------- Fetching instances for model: ', Model.modelName, 'with oldUpdateTime gte: ', timeToRecoverOld, ' ---------');
        fetchLimit(Model, queryOld, 'oldUpdateTime', function (err, MaxTimeOld) {
          if (err) {
            return callback(err);
          }
          ...
          console.log('--------- For model: ', Model.modelName, ' new maxTime in map for oldUpdateTime is: ', MaxTimeOld);
          if (typeof timeToRecoverCurr !== 'undefined') {
            ...
            console.log('--------- Fetching instances for model: ', Model.modelName, 'with currentUpdateTime gte: ', timeToRecoverCurr, ' ---------');
            fetchLimit(Model, queryCurr, 'currentUpdateTime', function (err, maxTimeCurr) {
              if (err) {
                return callback(err);
              }
              timeToRecoverPerModelMap[Model.modelName] = timeToRecoverPerModelMap[Model.modelName] || {};
              timeToRecoverPerModelMap[Model.modelName].timeToRecoverCurr = maxTimeCurr;
              console.log('--------- For model: ', Model.modelName, ' new maxTime in map for currentUpdateTime is: ', maxTimeCurr);
              return callback();
            });
          }
        });
        ...
        console.log('--------- Fetching instances for model: ', Model.modelName, 'with currentUpdateTime $$$$$$$$$$$$ gte: ', timeToRecoverCurr, ' ---------');
        fetchLimit(Model, query, 'currentUpdateTime', function (err, maxTimeCurr) {
          if (err) {
            return callback(err);
          }
          timeToRecoverPerModelMap[Model.modelName] = timeToRecoverPerModelMap[Model.modelName] || {};
