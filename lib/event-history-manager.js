...
function logFail(eventHistory, i) {
  // TODO 
  var instance = {};
  ...
  FailedObserverLog.create(instance, ignoreScopeOptions, function (err) {
    if (err) {
      log.error(log.defaultContext(), err.message);
      if (i < 10) setTimeout(logFail, 2000, eventHistory, ++i);
      else  {
...
function filterEvents() {
  ...
  for (var key of keys) {
    ...
    timeSinceLastReplay = new Date() - event.created;
...
function persistToDB() {
  getSavePointInstance().updateAttribute('timestamp',  getMinTimestamp(), ignoreScopeOptions, (err) => log.debug(log.defaultContext(), err));
}
...
function getSavePointInstance() {
  if (savePointInstance) return savePointInstance;
  ...
  SavePointModel.create(instance, ignoreScopeOptions, function (err, instance) {
    if (err || !instance) {
      log.debug(log.defaultContext(), err);
      return setTimeout(getSavePointInstance, 2000);
    }
