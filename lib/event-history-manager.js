...
function persistToDB() {
  ...
  if (_historyChanged) {
    ...
    eventHistoryModel.destroyAll({or: [
      {and: [{hostName: currHostName}, {status: 'undefined'}]},
      {and: [{hostName: currHostName}, {status: 'RecoveryFinished'}]}
...
function init(app) {
  ...
  eventHistoryModel.find({ where: { hostName: currHostName } }, ignoreScopeOptions, function (err, results) {
    ...
    async.each(results, function (instance, cb) {
      if (!instance.status) {
        eventHistoryModel.updateAttribute('status',  'ToBeRecovered', ignoreScopeOptions, function (err) {
          cb(err);
        });
      } else {
        cb();
...
function recovery(hostname, cb) {
  var eventHistoryModel = getEventHistoryModel();
  eventHistoryModel.find({ where: { and: [{ hostName: hostname }, { status: 'ToBeRecovered'}] }}, ignoreScopeOptions, function (err, results) {
    if (err) {
      ...
      async.each(results, function (instance, eventHistoryInstanceCB) {
        if (instance) {
          instance.updateAttribute('status',  'InRecovery', ignoreScopeOptions, function (err) {
            if (err) {
...
function fetchModels(eventHistoryInstance, hostNameToRecover, recoveryTimeScopePerModelMap, cb) {
  ...
  async.each(Object.keys(app.models), function (model, callbackFM) {
    ...
    if (err) {
      ...
      setTimeout(fetchModels.bind(this, hostNameToRecover, recoveryTimeScopePerModelMap, cb), SERVER_RECOVERY_INTERVAL);
    } else if (!recoveryAvailable[hostNameToRecover].recovery) {
      log.debug(log.defaultContext(), 'Recovery for ', hostNameToRecover, ' finished successfully!');
      thisInstance.updateAttribute('status',  'RecoveryFinished', ignoreScopeOptions, function (err) {
        if (err) {
          cb(err);
        } else {
