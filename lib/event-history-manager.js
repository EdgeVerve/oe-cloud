...
function createEventHistory(modelName, version, operation, ctx) {
  ...
  if (instance) {
    ...
    eventHistory.timestamp = instance.currentHNT.updateTime;
  }
  if (ctx.data) {
...
function init(app) {
  ...
  if (!process.env.USE_BROADCASTER) {
    recovery(currHostName, function (err) {
      if (err) {
        log.error(log.defaultContext(), 'failed to do recovery of server: ', currHostName, ' with error: ', err);
...
myEE.on('event reliability recovery', function (recoveryHostNameArr) {
  async.each(recoveryHostNameArr, function (hostname, asyncCb) {
    ...
    if (err) {
      log.error(log.defaultContext(), 'error while recovery of dead nodes', err);
      // TO DO - error handling here
    } else {
  ...
  eventHistoryModel.findOne({ where: { hostName: hostname } }, ignoreScopeOptions, function (err, instance) {
    if (err) {
      ...
      if (typeof instance.timestamp !== 'undefined' && instance.doStartUpOperation) {
        ...
        setTimeout(fetchModels.bind(this, hostNameToRecover, timeToRecoverPerModelMap), SERVER_RECOVERY_INTERVAL);
      }
    }
  });
}
...
function fetchLimit(Model, query, property, cb) {
  Model.find(query, { fetchAllScopes: true, ctx: { tenantId: 'default' } }, function (err, results) {
    if (err) {
      log.error(log.defaultContext(), 'failed to fetch instances from DB', err);
      return cb();
    }
    if (results) {
      ...
      if (results.length !== 0) {
        var maxTime = results[results.length - 1][property].updateTime;
      }
      async.each(results, function (rec, asyncCb) {
        ...
        rec.createEventHistory(context, context.ctx.options, function (err) {
          if (err) {
            log.error(log.defaultContext(), 'failed to create eventHistory remotely', err);
            // TODO error handling
            return asyncCb();
          }
...
function fetchModels(hostNameToRecover, timeToRecoverPerModelMap) {
  ...
  async.each(Object.keys(app.models), function (model, callback) {
    if (app.models[model].definition.settings.mixins &&
      ...
      if (typeof timeToRecoverOld !== 'undefined') {
        ...
        query.order = ['oldHNT.updateTime ASC'];
        query.where = {and: [{'oldHNT.updateTime': {gte: timeToRecoverOld}}, {'oldHNT.hostName': hostNameToRecover}]};
        fetchLimit(Model, query, 'oldHNT', function (err, MaxTimeOld) {
          if (err) {
            return callback(err);
          }
          ...
          if (timeToRecoverCurr !== 'undefined') {
            var query = {};
            query.limit = INSTANCES_TO_FETCH;
            query.order = ['currentHNT.updateTime ASC'];
            query.where = {and: [{'currentHNT.updateTime': {gte: timeToRecoverCurr}}, {'currentHNT.hostName': hostNameToRecover}]};
            fetchLimit(Model, query, 'currentHNT', function (err, maxTimeCurr) {
              if (err) {
                return callback(err);
              }
              timeToRecoverPerModelMap[Model.modelName] = timeToRecoverPerModelMap[Model.modelName] || {};
    ...
    if (err) {
      log.error(log.defaultContext(), 'error in fetching models', err);
      // TO DO - error handling here
