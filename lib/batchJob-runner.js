  ...
  var updateJobMonitoring = function (monitoringId, data, callback) {
    ...
    var acquireMonitorVersion = (token) => {
      ...
      request(requestOptions, (err, response, body) => {
        if (err || body.error) {
          log.error(log.defaultContext(), 'Error in requesting monitor instance. error: ', err || body.error);
        }
    ...
    var sendMonitoringUpdate = (data, token) => {
      ...
      return request(requestOptions, (err, response, body) => {
        if (err || body.error) {
          log.error(log.defaultContext(), 'Error in sending data to monitoring service. error: ', err || body.error);
        }
        callback();
      });
    };
